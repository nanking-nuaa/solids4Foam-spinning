...
#include "electricthermalLinGeomSolid.H"
#include "fvm.H"
#include "fvc.H"
...
defineTypeNameAndDebug(electricthermalLinGeomSolid, 0);
addToRunTimeSelectionTable(physicsModel, electricthermalLinGeomSolid, solid);
addToRunTimeSelectionTable(solidModel, electricthermalLinGeomSolid, dictionary);


// * * * * * * * * * * *  Private Member Functions * * * * * * * * * * * * * //

bool electricthermalLinGeomSolid::converged
(
    const int iCorr,
...
electricthermalLinGeomSolid::electricthermalLinGeomSolid
(
    Time& runTime,
    const word& region
)
...
bool electricthermalLinGeomSolid::evolve()
{
    Info<< "Evolving electricthermal solid solver" << endl;

    int iCorr = 0;
#ifdef OPENFOAMESIORFOUNDATION
    SolverPerformance<vector> solverPerfD;
    SolverPerformance<scalar> solverPerfT;
//    SolverPerformance<scalar> solverPerfG;
    SolverPerformance<vector>::debug = 0;
#else
    lduSolverPerformance solverPerfD;
    lduSolverPerformance solverPerfT;
//    lduSolverPerformance solverPerfG;
	...
	// Store fields for under-relaxation and residual calculation
        G().storePrevIter();
         ...
         fvScalarMatrix GEqn
          (
             rhoC_*fvm::ddt(G_)
           == fvm::laplacian(k2_, G_, "laplacian(k2,G)")
            + (sigma() && fvc::grad(U()))
          );
...
tmp<vectorField> electricthermalLinGeomSolid::tractionBoundarySnGrad
(
...
void electricthermalLinGeomSolid::writeFields(const Time& runTime)
{
